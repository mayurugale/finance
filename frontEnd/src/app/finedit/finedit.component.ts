...
export class FineditComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, public common: CommonService, public dialogRef: MatDialogRef<FineditComponent>,
    public router: Router, public tost: ToastrService, @Inject(MAT_DIALOG_DATA) public data: any) {
  ...
  onUpdate() {
    let obj = this.finupdate.value
    obj.attend = true
    this.common.putprodattend(obj, this.data.id).subscribe((result: any) => {
